					 ...
					 struct platform_device *pdev)
{
	...
	of_for_each_phandle(&it, err, np, "eirq-ranges", NULL, 3) {
		...
		if (!siul2_is_valid_pin(args[1])) {
			for (i = 0; i < args[2]; i++)
				gpio_dev->eirq_pins[index + i].pin = -1;
		} else {
			for (i = 0; i < args[2]; i++)
...
static int siul2_gpio_dir_in(struct gpio_chip *chip, unsigned int gpio)
{
	...
	ret = pinctrl_gpio_direction_input(chip, chip->base + gpio);
	if (ret)
		return ret;
			      ...
			      int val)
{
	...
	ret = pinctrl_gpio_direction_output(chip, chip->base + gpio);
	if (ret)
		return ret;
...
{
	return pinctrl_gpio_request(chip, chip->base + gpio_pin);
}

static void siul2_gpio_free(struct gpio_chip *chip, unsigned int offset)
{
	pinctrl_gpio_free(chip, chip->base + offset);
}
...
static int siul2_gpio_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	ret = pinctrl_gpio_direction_input(chip, gc->base + eirq);
	if (ret) {
			   ...
			   struct siul2_gpio_dev *gpio_dev)
{
	...
	gpio_dev->irqmap = syscon_regmap_lookup_by_phandle(pdev->dev.of_node,
							   "regmap2");
