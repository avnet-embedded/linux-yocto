...
static int spi_nor_sr_lock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	...
	if (!spi_nor_is_locked_sr(nor, ofs + len, params->size - (ofs + len),
				  status_old))
		can_be_top = false;
	...
	if (use_top)
		lock_len = params->size - ofs;
	else
		lock_len = ofs + len;

	if (lock_len == params->size) {
		val = mask;
	} else {
		min_prot_len = spi_nor_get_min_prot_length_sr(nor);
...
static int spi_nor_sr_unlock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	...
	if (!spi_nor_is_unlocked_sr(nor, ofs + len, params->size - (ofs + len),
