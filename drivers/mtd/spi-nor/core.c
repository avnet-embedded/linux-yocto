...
static int spi_nor_write_16bit_sr_and_check(struct spi_nor *nor, u8 sr1)
{
	...
	if (!(nor->flags & SNOR_F_NO_READ_CR)) {
		 ...
		 * consequence of the params->quad_enable() call.
		 *
...
#define OFFSET_16_MB 0x1000000
	...
	while (len) {
		...
		if (nor->read_proto == SNOR_PROTO_8_8_8_DTR)
			ret = spi_nor_octal_dtr_read(nor, addr, read_len, readbuf);
		else
				   ...
				   const u8 *buf)
{
	...
	tmp_buf = kmalloc(params->page_size, GFP_KERNEL);
	if (!tmp_buf)
		return -ENOMEM;

	memset(tmp_buf, 0xff, params->page_size);
