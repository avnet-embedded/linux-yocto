...
static int s32cc_wdt_probe(struct platform_device *pdev)
{
	int ret = 0;
...
static void s32cc_wdt_remove(struct platform_device *pdev)
{
...
static struct platform_driver s32cc_wdt_driver = {
	.probe = s32cc_wdt_probe,
	.remove_new = s32cc_wdt_remove,
...
module_platform_driver(s32cc_wdt_driver);
