			       ...
			       unsigned int len)
{
	...
	ctx->ecdh.curve_id = params.curve_id;
	/* Use stdrng to generate private key */
	if (!params.key || !params.key_size) {
		params.key = key;
		params.key_size = cpt_ecdh_curvesz_get(params.curve_id);
...
static int cpt_ecdh_init_tfm(struct crypto_kpp *tfm)
{
	struct cpt_asym_ctx *ctx = kpp_tfm_ctx(tfm);
	struct pci_dev *pdev;
	int ret, cpu_num;
...
static struct kpp_alg cpt_ecdh = {
...
int otx2_cpt_register_asym_algs(void)
{
	...
	ret = crypto_register_kpp(&cpt_ecdh);
	if (ret)
		goto unreg_rsa;
...
unreg_ecdh:
	crypto_unregister_kpp(&cpt_ecdh);
unreg_rsa:
	for (--i; i >= 0; --i)
