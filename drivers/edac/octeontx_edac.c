...
static void octeontx_edac_ea_msg(struct octeontx_ghes_record *rec, char msg[SIZE])
{
	...
	cper_print_proc_arm(pfx, desc, sizeof(rec->core));
...
static int octeontx_cpu_probe(struct platform_device *pdev)
{
	...
	edac_dev = edac_device_alloc_ctl_info(0, "cpu", cores, name, 1, 0, idx);
	if (!edac_dev)
		return -ENOMEM;
...
static void octeontx_mc_remove(struct platform_device *pdev)
{
...
static void octeontx_cpu_remove(struct platform_device *pdev)
{
	struct edac_device_ctl_info *edac_dev = platform_get_drvdata(pdev);
	struct device *dev = &pdev->dev;
	char *name = (char *)dev->driver->of_match_table->name;
