			       ...
			       int irq)
{
	...
	if (ret) {
		pr_err(STM_TIMER_NAME": Unable to allocate interrupt line\n");
		return ret;
	}
...
static int __init nxp_stm_timer_probe(struct platform_device *pdev)
{
	...
	if (stm->irq <= 0) {
		dev_err(dev, "Unable to parse and map interrupt\n");
		return -EINVAL;
	}
	...
	if (ret) {
		dev_err(dev, "Failed to enable STM global timer clock\n");
		return ret;
	}
