...
/**
 * dwc3_usb_reset_assert - asserts reset using reset framework
 * @rstc: pointer to reset_control
 *
 * Return: 0 on success or error on failure
 */
static int dwc3_usb_reset_assert(struct reset_control *rstc)
{
...
/**
 * dwc3_usb_reset_release - de-asserts reset using reset framework
 * @rstc: pointer to reset_control
 *
 * Return: 0 on success or error on failure
 */
static int dwc3_usb_reset_release(struct reset_control *rstc)
{
	unsigned long loop_time = msecs_to_jiffies(RST_TIMEOUT);
	unsigned long timeout;
...
static int dwc3_zynqmp_power_req(struct dwc3 *dwc, bool on)
{
	...
	if (on) {
		...
		dwc3_usb_reset_release(simple->resets);
		...
		dwc3_usb_reset_assert(simple->resets);
