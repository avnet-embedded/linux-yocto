...
static int check_pcie_clk(struct serdes *serdes)
{
	...
	if (ret) {
		dev_err(dev, "Failed to get PCIe clock rate\n");
		return ret;
	}
...
static int mode_to_pcs_lane(unsigned int mode, int pcs_instance)
{
...
static void serdes_remove(struct platform_device *pdev)
{
