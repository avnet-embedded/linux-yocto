				    ...
				    u32 *reaction_list, u32 size_r)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (reaction_list[i] & S32CC_FCCU_REACTION_NMI) {
			mask->nmi.mask[ncf_num] |= BIT(reg_fault_pos);
			reactions++;
		}

		if (reaction_list[i] & S32CC_FCCU_REACTION_EOUT) {
			mask->eout.mask[ncf_num] |= BIT(reg_fault_pos);
			reactions++;
		}

		if (reaction_list[i] & S32CC_FCCU_REACTION_FUNC_RESET) {
...
static inline u32 get_offset(struct fccu_ncf_regs *reg, u32 reg_index)
{
		...
		struct fccu_pri_data *priv_data)
{
	...
	for (i = 0; i < mask->status.size; i++) {
		...
		if (ret < 0)
			return ret;
		if (ret > 0) {
			dev_info(dev, "Cleared faults 0x%x from NCF_S%u (%d) state=%d->%d\n",
				 reg_ncf_val, i, ret,
...
static int fccu_probe(struct platform_device *pdev)
{
	struct fccu_pri_data *priv_data = NULL;
	struct device *dev = &pdev->dev;
	int ret;

	priv_data = devm_kzalloc(dev, sizeof(struct fccu_pri_data),
				 GFP_KERNEL);
	if (!priv_data)
		return -ENOMEM;
...
static void fccu_remove(struct platform_device *pdev)
{
	struct fccu_pri_data *priv_data = NULL;
...
static struct platform_driver fccu_driver = {
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
...
module_platform_driver(fccu_driver);

MODULE_AUTHOR("NXP");
MODULE_DESCRIPTION("FCCU driver for S32CC SoC");
MODULE_LICENSE("GPL");
