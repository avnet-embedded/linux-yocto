...
int octeontx2_spi_transfer_one_message(struct spi_controller *ctlr,
				       struct spi_message *msg)
{
	struct octeontx2_spi *p = spi_controller_get_devdata(ctlr);
			       ...
			       const struct pci_device_id *ent)
{
	...
	host = spi_alloc_host(dev, sizeof(struct octeontx2_spi));
	if (!host)
		return -ENOMEM;
