...
{
	int index = module - LLCE_TX;

	if (index < LLCE_TX || index >= ARRAY_SIZE(llce_modules))
		return "Unknown module";

	return llce_modules[index];
}

static int get_num_chans(struct device *dev)
{
	unsigned int temp = 0, i;
	int num = 0, ret;

	for (i = 0; i < ARRAY_SIZE(mb_map); i++) {
		ret = check_add_overflow(num, mb_map[i].nchan, &temp);
		if (ret) {
			dev_err(dev,
				"The total number of channels overflows num = %u mb_map[%d].nchan = %u\n",
				num, i, mb_map[i].nchan);
			return ret;
		}
		num += mb_map[i].nchan;
	}
...
static void disable_rxout_irq_by_index(struct llce_mb *mb, unsigned int index)
{
...
{
	uintptr_t addr, size = icsr_id * sizeof(u32);

	if (check_add_overflow((uintptr_t)mb->icsr, size, &addr))
		return NULL;
...
static void llce_process_tx_ack(struct llce_mb *mb, unsigned int index)
{
...
{
	unsigned int chan_index;
	enum llce_can_module module_id = LLCE_TX;
	enum llce_fw_return fw_error = LLCE_FW_SUCCESS;
	struct mbox_controller *ctrl = &mb->controller;
...
static void llce_process_rxin(struct llce_mb *mb, unsigned int index)
{
...
static void llce_process_rxout(struct llce_mb *mb, unsigned int index)
{
	void __iomem *rxout = get_rxout_by_index(mb, index);
	struct device *dev = mb->controller.dev;
	struct llce_rx_msg msg;
...
typedef void (*icsr_consumer_t)(struct llce_mb *, unsigned int);
...
static int llce_init_chan_map(struct device *dev, struct llce_mb *mb)
{
	...
	unsigned long fifo_id, ctrl_id = 0;
...
static int llce_mb_probe(struct platform_device *pdev)
{
	...
	ctrl->num_chans = get_num_chans(dev);
	if (ctrl->num_chans < 0)
		return ctrl->num_chans;
...
static void llce_mb_remove(struct platform_device *pdev)
{
