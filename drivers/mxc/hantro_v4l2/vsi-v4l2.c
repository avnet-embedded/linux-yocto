...
static int vsi_v4l2_dbg_instance(struct seq_file *s, void *data)
{
	...
	timems = div_u64(info->ts_last - info->ts_start, NSEC_PER_MSEC);
	fps_dec = div_u64(temp + (timems / 2), timems);
	if (info->total_time)
		{ u64 total_ms = div_u64(info->total_time, NSEC_PER_MSEC); fps_sw = div_u64(temp + (total_ms / 2), total_ms); }
	if (info->display_frame_num > 1) {
		temp = MSEC_PER_SEC * (info->display_frame_num - 1);
		timems = div_u64(info->ts_disp_last - info->ts_disp_first, NSEC_PER_MSEC);
		fps_dsp = div_u64(temp + (timems / 2), timems);
	}
	...
	if (seq_write(s, str, num))
		return 0;
	num = scnprintf(str, sizeof(str), " latency(ms): %llu.%06u\n",
			div_u64_rem(latency, NSEC_PER_MSEC, &latency_rem), latency_rem);
	if (seq_write(s, str, num))
		return 0;
...
void vsi_v4l2_reset_performance(struct vsi_v4l2_ctx *ctx)
{
	...
	v4l2_klog(LOGLVL_FLOW,
		  "[%llx]fps actual: %llu, disp: %llu, ideal: %llu, latency(ms) %llu.%06u\n",
