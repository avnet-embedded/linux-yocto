...
do {									\
	struct llist_node *n = &(x)->garbage.llist;			\
	n->next = n;							\
} while (0)
...
static int mpam_force_disable_open(struct inode *inode, struct file *file)
{
	return single_open(file, mpam_force_disable_show, inode->i_private);
}

static const struct file_operations mpam_force_disable_fops = {
	.owner		= THIS_MODULE,
	.open		= mpam_force_disable_open,
	.read		= seq_read,
	.write		= mpam_force_disable_write,
	.llseek		= seq_lseek,
