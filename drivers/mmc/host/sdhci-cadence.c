...
static int sdhci_cdns_sd6_phy_lock_dll(struct sdhci_cdns_priv *priv)
{
	...
	if (locked) {
		...
		if (delay_elements_in_sdmclk > 256) {
			DEBUG_DRV("%s: delay elements in sdmclk %u > 256, delay element: %u\n",
				  __func__, delay_elements_in_sdmclk,
				  delay_element);
			delay_element *= 2;
			...
			if (delay_elements_in_sdmclk > 256) {
				DEBUG_DRV("%s: WARNING: delay_elements_in_sdmclk %u "
					  "exceeds 256, clamping to 256\n",
					  __func__, delay_elements_in_sdmclk);
				delay_elements_in_sdmclk = 256;
			}
...
static int sdhci_cdns_sd6_get_delay_params(struct device *dev, struct sdhci_cdns_priv *priv)
{
	...
	ret = device_property_read_u32(dev, "cdns,max_tune_iter", &max_tune_iter);
	if (ret)
