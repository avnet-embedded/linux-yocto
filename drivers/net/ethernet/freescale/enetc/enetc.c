...
static bool enetc_tx_ring_avaiable(struct enetc_bdr *tx_ring, int num_txbd)
{
	struct enetc_ndev_priv *priv = netdev_priv(tx_ring->ndev);
	int max_supp_bd = ENETC_TXBDS_NEEDED(priv->max_frags_bd);
	int num_unused_bd = enetc_bd_unused(tx_ring);

	if (num_txbd > max_t(int, num_unused_bd, max_supp_bd))
		return false;
					  ...
					  int *xdp_tx_bd_cnt)
{
	...
	if (unlikely(!enetc_tx_ring_avaiable(tx_ring, nr_frags + 1)))
		return -EBUSY;
				   ...
				   struct bpf_prog *prog)
{
	...
	while (likely(rx_frm_cnt < work_limit)) {
		...
		case XDP_TX:
			...
			enetc_tx_queue_lock(tx_ring, cpu);
			if (unlikely(test_bit(ENETC_TX_DOWN, &priv->flags) ||
				    !enetc_tx_ring_avaiable(tx_ring, xdp_tx_bd_cnt))) {
				enetc_xdp_drop(rx_ring, orig_i, i);
				tx_ring->stats.xdp_tx_drops++;
				enetc_tx_queue_unlock(tx_ring);
...
static int enetc_get_xsk_buff_txbd_num(struct xdp_buff *xsk_buff)
{
				   ...
				   int work_limit)
{
	...
	while (likely(rx_frm_cnt < work_limit)) {
		...
		case XDP_TX:
			...
			if (unlikely(test_bit(ENETC_TX_DOWN, &priv->flags) ||
				     !enetc_tx_ring_avaiable(tx_ring, num_txbd))) {
				xsk_buff_free(xsk_buff);
				tx_ring->stats.xdp_tx_drops++;
				enetc_tx_queue_unlock(tx_ring);
				break;
...
static void enetc_xdp_rxq_info_unreg(struct enetc_bdr *rx_ring)
{
	if (xdp_rxq_info_is_reg(&rx_ring->xdp.rxq)) {
		xdp_rxq_info_unreg_mem_model(&rx_ring->xdp.rxq);
		xdp_rxq_info_unreg(&rx_ring->xdp.rxq);
	}
}

static int enetc_xdp_rxq_info_reg(struct enetc_bdr *rx_ring)
{
	struct xsk_buff_pool *pool = rx_ring->xdp.xsk_pool;
	enum xdp_mem_type type;
	int err;

	err = xdp_rxq_info_reg(&rx_ring->xdp.rxq, rx_ring->ndev,
			       rx_ring->index, 0);
	if (err)
		return err;
...
static int enetc_reconfigure_xdp_rxq_info(struct enetc_bdr *rx_ring)
{
	enetc_xdp_rxq_info_unreg(rx_ring);

	return enetc_xdp_rxq_info_reg(rx_ring);
}
...
static int enetc_reconfigure_xsk_cb(struct enetc_ndev_priv *priv, void *ctx)
{
	...
	err = enetc_reconfigure_xdp_rxq_info(rx_ring);
	if (err) {
