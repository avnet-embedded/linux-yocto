...
static void rvu_rep_get_stats(struct work_struct *work)
{
	...
	mutex_lock(&priv->mbox.lock);
	req = otx2_mbox_alloc_msg_nix_get_lf_stats(&priv->mbox);
	if (!req) {
		mutex_unlock(&priv->mbox.lock);
		return;
	}
...
int rvu_rep_create(struct otx2_nic *priv, struct netlink_ext_ack *extack)
{
	...
	for (rep_id = 0; rep_id < rep_cnt; rep_id++) {
		...
		snprintf(ndev->name, sizeof(ndev->name), "r%dpf%dvf%d", rep_id,
...
static int rvu_rep_rsrc_init(struct otx2_nic *priv)
{
	...
	priv->rbsize = ALIGN(priv->hw.rbuf_fixed_size, OTX2_ALIGN) + OTX2_HEAD_ROOM;
