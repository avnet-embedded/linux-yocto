...
static void otx2_rfoe_update_tx_drop_stats(struct otx2_rfoe_ndev_priv *priv,
					   int pkt_type)
{
...
static void otx2_rfoe_update_tx_stats(struct otx2_rfoe_ndev_priv *priv,
	...
	for (i = 0; i < RFOE_MAX_INTF; i++) {
		...
		if (drv_ctx->valid) {
			...
			if (!priv->ptp_cfg->refcnt) {
				timer_delete_sync(&priv->ptp_cfg->ptp_timer);
				kfree(priv->ptp_cfg);
			}
...
{
	struct ptp_bcn_off_cfg *ptp_cfg = timer_container_of(ptp_cfg, t, ptp_timer);
					    ...
					    struct net_device *netdev)
{
	...
	if (proto == ETH_P_1588)
		pkt_stats_type = PACKET_TYPE_PTP;
	else if (proto == ETH_P_ECPRI)
		pkt_stats_type = PACKET_TYPE_ECPRI;
	else
		pkt_stats_type = PACKET_TYPE_OTHER;
	...
	if (unlikely(priv->if_type != IF_TYPE_ETHERNET)) {
		...
		/* update stats */
		otx2_rfoe_update_tx_drop_stats(priv, pkt_stats_type);
		goto exit;
	}
	...
	if (unlikely(skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP) &&
		...
		if (list_empty(&priv->ptp_skb_list.list) &&
			...
			priv->ptp_skb_list.count++;
			skb_shinfo(skb)->tx_flags |= SKBTX_IN_PROGRESS;
			otx2_rfoe_update_tx_stats(priv, pkt_stats_type, skb->len);
			/* sw timestamp */
...
err_exit:
	...
	timer_delete_sync(&ptp_cfg->ptp_timer);
	kfree(ptp_cfg);
