...
/* global driver ctx */
struct cnf10k_cpri_drv_ctx cnf10k_cpri_drv_ctx[CNF10K_BPHY_CPRI_MAX_INTF];

/* debugfs */
static void cnf10k_cpri_debugfs_reader(char *buffer, size_t count, void *priv);
static const char *cnf10k_cpri_debugfs_get_formatter(void);
static size_t cnf10k_cpri_debugfs_get_buffer_size(void);
...
static struct net_device *cnf10k_cpri_get_netdev(int mhab_id, int lmac_id)
{
	...
	for (idx = 0; idx < CNF10K_BPHY_CPRI_MAX_INTF; idx++) {
		if (cnf10k_cpri_drv_ctx[idx].cpri_num == mhab_id &&
		    cnf10k_cpri_drv_ctx[idx].lmac_id == lmac_id &&
...
void cnf10k_cpri_enable_intf(int cpri_num)
{
	...
	struct cnf10k_cpri_ndev_priv *priv;
	struct net_device *netdev;
	int idx;

	for (idx = 0; idx < CNF10K_BPHY_CPRI_MAX_INTF; idx++) {
		drv_ctx = &cnf10k_cpri_drv_ctx[idx];
...
{
	struct cnf10k_cpri_drv_ctx *drv_ctx = NULL;
	struct cnf10k_cpri_ndev_priv *priv;
	struct net_device *netdev;
	int i;
	...
	for (i = 0; i < CNF10K_BPHY_CPRI_MAX_MHAB; i++)
		writeq(0x1, cpri_reg_base +
		       CNF10K_CPRIX_ETH_UL_INT_ENA_W1C(i));
}

static int cnf10k_cpri_process_rx_pkts(struct cnf10k_cpri_ndev_priv *priv,
				       int budget)
{
	...
	nxt_wr_ptr = readq(priv->cpri_reg_base +
			...
			0xFFFF;
	/* get the HW head */
	head = CNF10K_CIRC_BUF_ENTRY(nxt_wr_ptr);
	...
	while (likely((processed_pkts < budget) && (processed_pkts < count))) {
		...
		if (unlikely(netif_msg_pktdata(priv2))) {
			...
			print_hex_dump(KERN_DEBUG, "", DUMP_PREFIX_OFFSET, 16,
				       4, pkt_buf,
				       len + CNF10K_BPHY_CPRI_WQE_SIZE, true);
		}

		pkt_buf += CNF10K_BPHY_CPRI_WQE_SIZE;
...
/* napi poll routine */
static int cnf10k_cpri_napi_poll(struct napi_struct *napi, int budget)
{
	...
	priv = container_of(napi, struct cnf10k_cpri_ndev_priv, napi);
	cdev_priv = priv->cdev_priv;

	/* pkt processing loop */
	workdone += cnf10k_cpri_process_rx_pkts(priv, budget);

	if (workdone < budget) {
		napi_complete_done(napi, workdone);

		/* Re enable the Rx interrupts */
		intr_en = 1 << CNF10K_CPRI_RX_INTR_SHIFT(priv->cpri_num);
		spin_lock(&cdev_priv->lock);
		regval = readq(priv->bphy_reg_base + PSM_INT_GP_ENA_W1S(2));
		regval |= intr_en;
		writeq(regval, priv->bphy_reg_base + PSM_INT_GP_ENA_W1S(2));
...
void cnf10k_cpri_rx_napi_schedule(int cpri_num, u32 status)
{
	...
	for (idx = 0; idx < CNF10K_BPHY_CPRI_MAX_INTF; idx++) {
		...
		if (test_bit(CNF10K_CPRI_INTF_DOWN, &priv->state))
			continue;
		/* clear intr enable bit, re-enable in napi handler */
		regval = 1 << CNF10K_CPRI_RX_INTR_SHIFT(cpri_num);
		writeq(regval, priv->bphy_reg_base + PSM_INT_GP_ENA_W1C(2));
		/* schedule napi */
...
{
	struct cnf10k_cpri_stats *dev_stats = &priv->stats;

	dev_stats->rx_frames += readq(priv->cpri_reg_base +
				      CNF10K_CPRIX_ETH_UL_GPKTS_CNT(priv->cpri_num,
								    priv->lmac_id));
	dev_stats->rx_octets += readq(priv->cpri_reg_base +
				      CNF10K_CPRIX_ETH_UL_GOCT_CNT(priv->cpri_num,
								   priv->lmac_id));
	dev_stats->rx_err += readq(priv->cpri_reg_base +
				      CNF10K_CPRIX_ETH_UL_ERR_CNT(priv->cpri_num,
								  priv->lmac_id));
	dev_stats->bad_crc += readq(priv->cpri_reg_base +
				    CNF10K_CPRIX_ETH_BAD_CRC_CNT(priv->cpri_num,
								 priv->lmac_id));
	dev_stats->oversize += readq(priv->cpri_reg_base +
				     CNF10K_CPRIX_ETH_UL_OSIZE_CNT(priv->cpri_num,
								   priv->lmac_id));
	dev_stats->undersize += readq(priv->cpri_reg_base +
				      CNF10K_CPRIX_ETH_UL_USIZE_CNT(priv->cpri_num,
								    priv->lmac_id));
	...
	dev_stats->tx_octets += readq(priv->cpri_reg_base +
				      CNF10K_CPRIX_ETH_DL_GOCTETS_CNT(priv->cpri_num,
								      priv->lmac_id));
}

static void cnf10k_cpri_get_stats64(struct net_device *netdev,
...
/* netdev ioctl */
static int cnf10k_cpri_ioctl(struct net_device *netdev, struct ifreq *req,
			     int cmd)
{
	return -EOPNOTSUPP;
}
...
/* netdev xmit */
static netdev_tx_t cnf10k_cpri_eth_start_xmit(struct sk_buff *skb,
					      struct net_device *netdev)
{
	struct cnf10k_cpri_ndev_priv *priv = netdev_priv(netdev);
	...
	while ((readq(priv->cpri_reg_base +
		      CNF10K_CPRIX_TXD_GMII_DL_WR_DOORBELL(priv->cpri_num)) & 0xFF))
		cpu_relax();

	nxt_rd_ptr = readq(priv->cpri_reg_base +
			   CNF10K_CPRIX_TXD_GMII_DL_NXT_RD_PTR(priv->cpri_num)) &
			0xFFFF;
	/* get the HW tail */
	...
	buf_ptr += CNF10K_BPHY_CPRI_WQE_SIZE;
	...
	writeq(1, priv->cpri_reg_base +
	       CNF10K_CPRIX_TXD_GMII_DL_WR_DOORBELL(priv->cpri_num));
...
/* netdev open */
static int cnf10k_cpri_eth_open(struct net_device *netdev)
{
	struct cnf10k_cpri_ndev_priv *priv = netdev_priv(netdev);
	...
	spin_lock(&priv->lock);
	clear_bit(CNF10K_CPRI_INTF_DOWN, &priv->state);
...
/* netdev close */
static int cnf10k_cpri_eth_stop(struct net_device *netdev)
{
	struct cnf10k_cpri_ndev_priv *priv = netdev_priv(netdev);

	spin_lock(&priv->lock);
	set_bit(CNF10K_CPRI_INTF_DOWN, &priv->state);
...
static const struct net_device_ops cnf10k_cpri_netdev_ops = {
	.ndo_open		= cnf10k_cpri_eth_open,
	.ndo_stop		= cnf10k_cpri_eth_stop,
	.ndo_start_xmit		= cnf10k_cpri_eth_start_xmit,
	.ndo_eth_ioctl		= cnf10k_cpri_ioctl,
	.ndo_set_mac_address	= eth_mac_addr,
	.ndo_validate_addr	= eth_validate_addr,
	.ndo_get_stats64	= cnf10k_cpri_get_stats64,
};
				    ...
				    struct cnf10k_bphy_cpri_netdev_comm_intf_cfg *cfg)
{
	struct cnf10k_bphy_ndev_cpri_intf_cfg *cpri_cfg;
	struct cnf10k_cpri_drv_ctx *drv_ctx = NULL;
	struct cnf10k_cpri_ndev_priv *priv, *priv2;
	int i, intf_idx = 0, lmac, ret;
	struct net_device *netdev;

	for (i = 0; i < CNF10K_BPHY_CPRI_MAX_MHAB; i++) {
		priv2 = NULL;
		cpri_cfg = &cfg->cpri_if_cfg[i];
		for (lmac = 0; lmac < CNF10K_BPHY_CPRI_MAX_LMAC; lmac++) {
			if (!(cpri_cfg->active_lane_mask & (1 << lmac)))
				continue;
			netdev =
			    alloc_etherdev(sizeof(struct cnf10k_cpri_ndev_priv));
			if (!netdev) {
				dev_err(cdev->dev,
			...
			if (!priv2) {
				cnf10k_cpri_fill_dl_ul_cfg(priv, cpri_cfg);
			} else {
			...
			netif_napi_add(priv->netdev, &priv->napi,
				       cnf10k_cpri_napi_poll, NAPI_POLL_WEIGHT);
			...
			snprintf(netdev->name, sizeof(netdev->name),
				 "cpri%d", intf_idx);
			netdev->netdev_ops = &cnf10k_cpri_netdev_ops;
			cnf10k_cpri_set_ethtool_ops(netdev);
			netdev->mtu = 1500U;
			...
			drv_ctx = &cnf10k_cpri_drv_ctx[intf_idx];
			...
			cnf10k_cpri_debugfs_create(drv_ctx);
		}
	}
...
static void cnf10k_cpri_debugfs_reader(char *buffer, size_t count, void *priv)
{
	struct cnf10k_cpri_ndev_priv *netdev;
	struct cnf10k_cpri_drv_ctx *ctx;
	...
	formatter = cnf10k_cpri_debugfs_get_formatter();
...
static size_t cnf10k_cpri_debugfs_get_buffer_size(void)
{
	...
	if (!buffer_size) {
		const char *formatter = cnf10k_cpri_debugfs_get_formatter();
		u8 max_boolean = 1;
...
static void cnf10k_cpri_debugfs_create(struct cnf10k_cpri_drv_ctx *ctx)
{
	size_t buffer_size = cnf10k_cpri_debugfs_get_buffer_size();

	ctx->debugfs = otx2_bphy_debugfs_add_file(ctx->netdev->name,
						  buffer_size, ctx,
						  cnf10k_cpri_debugfs_reader);
}
