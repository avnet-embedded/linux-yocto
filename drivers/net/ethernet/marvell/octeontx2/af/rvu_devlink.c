...
static irqreturn_t rvu_nix_af_rvu_err_handler(int irq, void *rvu_irq)
{
	...
	dev_err_ratelimited(rvu->dev, "NIX_AF_ERR_INT = 0x%llx\n", intr);
...
static irqreturn_t rvu_nix_af_rvu_ras_handler(int irq, void *rvu_irq)
{
	...
	dev_err_ratelimited(rvu->dev, "NIX_AF_RAS = 0x%llx\n", intr);
					  ...
					  struct devlink_param_gset_ctx *ctx)
{
	...
	if (offset == TIM_AF_CAPTURE_PTP && rvu_tim_ptp_has_errata(rvu->pdev))
		time = rvu_tim_ptp_rollover_errata_fix(rvu, time);
