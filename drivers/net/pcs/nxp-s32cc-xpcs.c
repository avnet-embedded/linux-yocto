...
#define SPEED_SELECTION_10Mbps			0
#define SPEED_SELECTION_100Mbps			1
#define SPEED_SELECTION_1000Mbps		2
...
#define phylink_pcs_to_s32cc_xpcs(pl_pcs) \
	container_of((pl_pcs), struct s32cc_xpcs, pcs)
...
static void init_params(u32 reg, struct s32cc_xpcs *xpcs,
			struct s32cc_xpcs_params *params, u16 *data)
{
			  ...
			  struct phylink_link_state *state)
{
	...
	switch (ss) {
	case SPEED_SELECTION_10Mbps:
		state->speed = SPEED_10;
		break;
	case SPEED_SELECTION_100Mbps:
		state->speed = SPEED_100;
		break;
	case SPEED_SELECTION_1000Mbps:
